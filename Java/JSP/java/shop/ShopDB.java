...
public class ShopDB {
    ...
    public void order(Basket basket , String customer) {
        try {
            ...
            for (Map.Entry<Product,Integer> entry : basket.items.entrySet()) {
                // and place the order for each one

                    order(con,entry.getKey(),orderId,customer,entry.getValue());
    ...
    private void order(Connection con, Product p, String orderId, String customer, Integer quantity) throws Exception {

        PreparedStatement query = con.prepareStatement("INSERT INTO ORDERS VALUES(?,?,?,?,?)");
        query.setString(1,p.PID);
        query.setString(2,orderId);
        query.setString(3,customer);
        query.setDouble(4,(double)quantity);
        query.setDouble(5, p.price);
